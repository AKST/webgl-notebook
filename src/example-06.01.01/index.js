...
export function main () {
  ...
  const state = initControls({
    ...
    entity: {
      translate: [0.00, -73.60, 68.30],
      rotation: [59.00, 59.16, 49.20],
      scale: [1.02, 1.25, 1.07],
    },
  });
  ...
  requestAnimationFrame(function f (t) {
    ...
    const count = 5, cameraAngle = t / 1000;
    let cameraM = M.rotateY(cameraAngle);
    cameraM = m.mul(M.translate(0, 0, radius.value * 1.5), cameraM);
    cameraM = M.lookat(
      v(3)(cameraM.mat[3][0], cameraM.mat[3][1], cameraM.mat[3][2]),
      math.el.add(
        v(3)(radius.value, 0, 0),
    ...
    for (let ii = 0; ii < count; ++ii) {
      ...
      for (let i = 0; i < countIn.value; i++) {
        matrix = m.mul(tM, matrix);
        matrix = m.mul(M.translate(x, 0, z), matrix);
