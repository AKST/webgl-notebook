...
const vertexShaderSrc = `
  ...
  void main() {
    vec2 s_position = a_position * u_scale;
    vec2 r_position = vec2(
      s_position.x * u_rotation.y + s_position.y * u_rotation.x,
      s_position.y * u_rotation.y - s_position.x * u_rotation.x);

    vec2 position = r_position + u_translation;
...
export function main () {
  ...
  const uRotLoc = gl.getUniformLocation(program, "u_rotation")
  console.log(uRotLoc, vertexShaderSrc)
  if (uRotLoc == null) throw new Error("u_rotation");

  const uResLoc = gl.getUniformLocation(program, "u_resolution")
  if (uResLoc == null) throw new Error("u_resolution");

  const uSclLoc = gl.getUniformLocation(program, "u_scale")
  if (uSclLoc == null) throw new Error("u_scale");
  ...
  requestAnimationFrame(function f () {
    ...
    gl.uniform2fv(uRotLoc, [Math.sin(state.rotation), Math.cos(state.rotation)]);
    gl.uniform2fv(uResLoc, [gl.canvas.width, gl.canvas.height]);
    gl.uniform2fv(uSclLoc, state.scale);
